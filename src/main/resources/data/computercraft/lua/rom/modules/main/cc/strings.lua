...
local text_len, text_sub, byte_len = unicode.len, unicode.sub, unicode.byte_len
...
local function wrap(text, width)
    ...
    local pos, length = 1, byte_len(text)
    ...
    while pos <= length do
        ...
        if head == " " or head == "\t" then
            ...
            pos = pos + byte_len(whitespace)
        elseif head == "\n" then
            push_line()
            pos = pos + 1
        else
            local word = match(text, "^[^ \t\n]+", pos)
            pos = pos + byte_len(word)
