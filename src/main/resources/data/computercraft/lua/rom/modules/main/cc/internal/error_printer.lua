...
return function(context, message)
    ...
    for msg_idx = 1, #message do
        ...
        if type(msg) == "table" and msg.tag == "annotate" then
            ...
            -- Parser locations are byte offsets, while terminal columns are
            -- character cells. Convert them before calculating the viewport
            -- and annotation positions.
            col = text_len(contents:sub(1, col - 1)) + 1
            if line == end_line then
                end_col = text_len(contents:sub(1, end_col))
            end
            ...
            --  - The end of the line.
            local contents_len = text_len(contents)
            if line ~= end_line then end_col = contents_len end
            local start_col = math.max(1, math.min(col + 10, end_col + 5, contents_len + 1) - width + 1)
            ...
            if str_end < contents_len then
                str_end = str_end - 1
                suffix = pretty.text("\xbb", colours.grey)
            end

            pretty.print(code_accent .. pretty.text("Line " .. line, colours.cyan))
            pretty.print(code_accent .. prefix .. pretty.text(text_sub(contents, str_start, str_end), colours.lightGrey) .. suffix)
