...
local function parse_markdown(text)
    ...
    local function append(txt, fore, back)
        ...
        local width = text_len(txt)
        fg = fg .. (fore or "0"):rep(width)
        bg = bg .. (back or "f"):rep(width)
    end
...
local function word_wrap_markdown(text, width)
    ...
    for k, line in pairs(lines) do
        ...
        local cell_pos = text_len(text:sub(1, pos - 1)) + 1
        local line_width = text_len(line)
        lines[k], fglines[k], bglines[k] =
            strings.ensure_width(line, width),
            strings.ensure_width(fg:sub(cell_pos, cell_pos + line_width - 1), width),
            strings.ensure_width(bg:sub(cell_pos, cell_pos + line_width - 1), width)
